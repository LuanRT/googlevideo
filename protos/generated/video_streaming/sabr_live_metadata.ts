...
//   protoc               v7.35.1
// source: video_streaming/sabr_live_metadata.proto
...
export interface SabrLiveMetadata {
...
function createBaseSabrLiveMetadata(): SabrLiveMetadata {
  return {
    broadcastId: "",
...
export const SabrLiveMetadata: MessageFns<SabrLiveMetadata> = {
  encode(message: SabrLiveMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): SabrLiveMetadata {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseSabrLiveMetadata();
